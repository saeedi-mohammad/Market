 ...
 * <p>
 * Note: for formatting gregorian date based on {@link java.time.LocalDate} you
 * have to use {@link DateTimeFormatter}.
 *
...
public class JalaliDateFormatter {
	 ...
	 *
	 * <p>
	 * you can call it from outside of the class for setting formatting language.
	 * usually can be used in constructor
	 */
	 ...
	 *
	 * @param pattern        desired pattern.
	 * @param formatLanguage shows the selected language which 1 is persian and 2 is
	 *                       english.
	 */
	...
	/**
	 * Returns a string that shows given jalali date in a proper formatting, it
	 * first parses the given pattern and creates some place holders for future
	 * values then it applies format.
	 *
	...
	private String applyFormat(int index) {
		...
		case DateFormatterTemporalField.MONTH_STRING_FIElD:
			String monthString = formatLanguage == FORMAT_IN_PERSIAN ? jalaliDate.getMonthPersian().getStringInPersian()
					: jalaliDate.getMonthPersian().getStringInEnglish();
	 ...
	 * <p>
	 * It changes each english number with its persian version, for example 1
	 * becomes ۱.
	 *
	...
	// validate entered pattern and then inflate the array list of place holders (
	// temporal fields )
	...
	// an array list of place holders for each pattern element, each pattern element
	// will be mapped to one temporal field
	private ArrayList<DateFormatterTemporalField> dateFormatterTemporalFields = new ArrayList<>();

	/*
	 * according to position and numbers of each pattern element ( like mm ), it
	 * creates one temporal field and add it to list of temporal fields.
	 */
	private void parseField(char cur, int count) {
		...
		case 'y':
			...
			if (count == 2) {
				dateFormatterTemporalFields
						.add(new DateFormatterTemporalField(2, DateFormatterTemporalField.YEAR_FIElD));
			} else if (count == 4) {
				dateFormatterTemporalFields
						.add(new DateFormatterTemporalField(4, DateFormatterTemporalField.YEAR_FIElD));
			} else {
		...
		case 'm':
			if (count == 1) {
				dateFormatterTemporalFields
						.add(new DateFormatterTemporalField(1, DateFormatterTemporalField.MONTH_FIElD));
			} else if (count == 2) {
				dateFormatterTemporalFields
						.add(new DateFormatterTemporalField(2, DateFormatterTemporalField.MONTH_FIElD));
			} else {
				throw new IllegalArgumentException("Too many pattern letters: " + cur);
			}
			break;
		...
		case 'M':
			if (count == 1) {
				dateFormatterTemporalFields
						.add(new DateFormatterTemporalField(1, DateFormatterTemporalField.MONTH_STRING_FIElD));
			} else {
				throw new IllegalArgumentException("Too many pattern letters: " + cur);
			}
			break;
		...
		case 'd':
			if (count == 1) {
				dateFormatterTemporalFields
						.add(new DateFormatterTemporalField(1, DateFormatterTemporalField.DAY_FIElD));
			} else if (count == 2) {
				dateFormatterTemporalFields
						.add(new DateFormatterTemporalField(2, DateFormatterTemporalField.DAY_FIElD));
			} else {
				throw new IllegalArgumentException("Too many pattern letters: " + cur);
			}
			break;
		...
		case ' ':
			dateFormatterTemporalFields
					.add(new DateFormatterTemporalField(count, DateFormatterTemporalField.SPACE_FIElD));
			break;
		case '-':
			dateFormatterTemporalFields
					.add(new DateFormatterTemporalField(count, DateFormatterTemporalField.DASH_FIElD));
			break;
		case '/':
			dateFormatterTemporalFields
					.add(new DateFormatterTemporalField(count, DateFormatterTemporalField.SLASH_FIElD));
			break;
		}
	}
	 ...
	 * <p>
	 * y: year (Note: use yyyy) m: month in number (Note: use mm) M: month in word
	 * (Note: use M) d: day (Note: use dd) ' ', '/' and '-' can be used as a
	 * separator.
	 */
