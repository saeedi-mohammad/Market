 ...
 * <p>
 * This class holds some data about specific date in Jalali's Date format like
 * year, month, day, day of week and leap year
 *
...
public class JalaliDate {
	...
	/**
	 * Only DateConverter is allowed to create an object of JalaliDate with empty
	...
	public JalaliDate(int year, int month, int day) {
		...
		if (!leapYear && month == 12 && day >= 30)
			throw new IllegalArgumentException(
					"Wrong value for day, just in leap year last month must be greater than 29");
	...
	private void basicArgumentValidating(int year, int month, int day) {
		...
		if (month >= 7 && month <= 12 && day == 31)
			throw new IllegalArgumentException(
					"Wrong value for day. in second half of year, months have less than 31 days ");
	}

	/**
	 * reformats jalali date with provided format. the result will be a string in
	 * either persian or english.
	 *
	 * @param jalaliDateFormatter an object that shows preferred format for jalali
	 *                            date.
	 ...
	 * <p>
	 * Note: only classes of this package needs this method, you'll never need it.
	 ...
	 *
	 * @return DayOfWeekPersian object will be returned that user can get its
	 *         persian or english string version by calling getStringInXXX() method.
	 */
	 ...
	 * <p>
	 * Compares this {@link JalaliDate} with another for ensuring that the date is
	 * the same.
	 * <p>
	 * Only object type of {@link JalaliDate} are compared, other types return
	 * false.
	 *
